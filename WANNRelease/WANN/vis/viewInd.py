...
def viewInd(ind, taskName):
    ...
    # Draw Graph
    fig = plt.figure(figsize=(1920, 1080), dpi=300)
    ax = fig.add_subplot(111)
    drawEdge(G, pos, wMat, layer)
    nx.draw_networkx_nodes(G, pos, node_color='lightblue', node_shape='o', cmap='terrain', vmin=0, vmax=6)
    # plt.savefig(fname='01_wann_run_mnist_aws_connections.png', format='png', dpi=300, quality=100)
...
def labelInOut(pos, env):
    ...
    if len(env.in_out_labels) > 0:
        ...
        labelDict = {}
    print('state labels', stateLabels)
...
def drawNodeLabels(G, pos, aVec):
    actLabel = np.array(
        (['', '( + )', '(0/1)', '(sin)', '(gau)', '(tanh)', '(sig)', '( - )', '(abs)', '(relu)', '(cos)']))
